...
def entity_key(entity_type, entity_id):
    if entity_type is None or entity_id is None:
        return None
    return str(entity_type).strip().lower() + ":" + str(entity_id).strip()
...
def add_label(driver, entity_type):
    ...
    """
    with driver.session(database=NEO4J_DATABASE) as session:
        session.run(query, entity_type=str(entity_type).lower()).consume()
...
def load_table(driver, conn, table, id_column, entity_type, step_number):
    ...
    for row in rows:
        ...
        if item_id is None:
            continue
        data.append({
            "entity_key": entity_key(entity_type, item_id),
...
def load_actors(driver, conn):
    ...
    print(f"Loaded {len(rows):,} actors.")
...
def load_wallet_transactions(driver, conn):
    ...
    for row in rows:
        if row.get("from_wallet"):
            wallets.add(str(row["from_wallet"]))
        if row.get("to_wallet"):
            wallets.add(str(row["to_wallet"]))

    wallet_entities = [
        {"entity_key": entity_key("wallet", w), "entity_type": "wallet", "entity_id": w}
        for w in wallets
    ]
    create_entities(driver, wallet_entities)
    add_label(driver, "wallet")
    print(f"Loaded {len(wallet_entities):,} wallets.")
    ...
    for row in rows:
        ...
        if tid is not None:
            transaction_entities.append({
                "entity_key": entity_key("transaction", tid),
                "entity_type": "transaction",
                "entity_id": str(tid),
            })

    create_entities(driver, transaction_entities)
    ...
    """
    data = [
        {"entity_key": entity_key("transaction", r["transaction_id"]), "properties": clean_row(r)}
        for r in rows
    ...
    with driver.session(database=NEO4J_DATABASE) as session:
        ...
        for batch in chunks(data, BATCH_SIZE):
            session.run(query, rows=batch).consume()
            total += len(batch)
            print(f"  Transactions: {total:,}/{len(data):,}")

    paths = []
    for row in rows:
        tid = row.get("transaction_id")
        from_w = row.get("from_wallet")
        to_w = row.get("to_wallet")
        if tid and from_w and to_w:
            paths.append({
                "transaction_id": str(tid),
                "from_wallet": str(from_w),
                "to_wallet": str(to_w),
            })
    ...
    with driver.session(database=NEO4J_DATABASE) as session:
        ...
        for batch in chunks(paths, BATCH_SIZE):
            ...
            total += len(batch)
            print(f"  Wallet transaction paths: {total:,}/{len(paths):,}")
...
def load_relationships(driver, conn):
    ...
    print(f"Read {len(rows):,} relationships.")
    endpoint_map = {}
    for row in rows:
        sk = entity_key(row["source_entity_type"], row["source_entity_id"])
        tk = entity_key(row["target_entity_type"], row["target_entity_id"])
        if sk:
            endpoint_map[sk] = {
                "entity_key": sk,
                "entity_type": str(row["source_entity_type"]).lower(),
    ...
    create_entities(driver, endpoints)
    entity_types = sorted({x["entity_type"] for x in endpoints})
    for et in entity_types:
        add_label(driver, et)

    print(f"Prepared {len(endpoints):,} unique relationship endpoints.")
    ...
    for row in rows:
        ...
        grouped.setdefault(rel_type, []).append({
            ...
            "source_key": sk,
            "target_key": tk,
            "source_type": str(row["source_entity_type"]).lower(),
            "target_type": str(row["target_entity_type"]).lower(),
            "source_id": str(row["source_entity_id"]),
            "target_id": str(row["target_entity_id"]),
            "source_id_original": clean_value(row["source_id"]),
            "event_timestamp": clean_value(row["event_timestamp"]),
            "confidence": clean_value(row["confidence"]),
        })

    for rel_type in sorted(grouped):
        rel_rows = grouped[rel_type]
        print(f"\n{rel_type}: {len(rel_rows):,}")
        query = f"""
        UNWIND $rows AS row
        MATCH (source:Entity {{ entity_key: row.source_key }})
        MATCH (target:Entity {{ entity_key: row.target_key }})
        MERGE (source)-[r:{rel_type} {{ relationship_id: row.relationship_id }}]->(target)
        SET r.source_type = row.source_type,
            r.target_type = row.target_type,
            r.source_id = row.source_id,
            r.target_id = row.target_id,
            r.source_id_original = row.source_id_original,
        ...
        with driver.session(database=NEO4J_DATABASE) as session:
            ...
            for batch in chunks(rel_rows, BATCH_SIZE):
                session.run(query, rows=batch).consume()
                total += len(batch)
                print(f"  {total:,}/{len(rel_rows):,}")

    print(f"\nLoaded {len(rows) - skipped:,} relationships.")
    if skipped:
...
def verify_graph(driver):
    ...
    queries = [
        ...
        ("Actors", "MATCH (n:Actor) RETURN count(n) AS count"),
        ("Handles", "MATCH (n:Handle) RETURN count(n) AS count"),
        ...
        ("Identifiers", "MATCH (n:Identifier) RETURN count(n) AS count"),
        ("Infrastructure", "MATCH (n:Infrastructure) RETURN count(n) AS count"),
        ("Wallets", "MATCH (n:Wallet) RETURN count(n) AS count"),
        ("Transactions", "MATCH (n:Transaction) RETURN count(n) AS count"),
        ("Activity events", "MATCH (n:ActivityEvent) RETURN count(n) AS count"),
        ("PGP nodes", "MATCH (n:PGP) RETURN count(n) AS count"),
        ("Domain nodes", "MATCH (n:Domain) RETURN count(n) AS count"),
    ]
    with driver.session(database=NEO4J_DATABASE) as session:
        for name, query in queries:
            try:
                result = session.run(query).single()
                print(f"{name:<25}{result['count']:,}")
...
def main():
    print("=" * 65)
    print("SIH DARK WEB PROJECT: POSTGRESQL -> NEO4J GRAPH LOADER")
    print("=" * 65)
    ...
    try:
        ...
        clear_graph(neo4j)
        load_actors(neo4j, postgres)
        load_table(neo4j, postgres, "sources", "source_id", "source", 2)
        load_table(neo4j, postgres, "posts", "post_id", "post", 3)
        load_table(neo4j, postgres, "observations", "observation_id", "observation", 4)
        load_table(neo4j, postgres, "identifiers", "identifier_id", "identifier", 5)
        load_table(neo4j, postgres, "infrastructure", "indicator_id", "infrastructure", 6)
        load_wallet_transactions(neo4j, postgres)
        load_table(neo4j, postgres, "activity_timeline", "event_id", "event", 8)
        load_relationships(neo4j, postgres)
        verify_graph(neo4j)
        print("\n" + "=" * 65)
